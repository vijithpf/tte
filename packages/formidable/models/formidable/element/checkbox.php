...
class FormidableElementCheckbox extends FormidableElement {
	
	public $element_text = 'Checkbox List';
	public $element_type = 'checkbox';
	public $element_group = 'Basic Elements';
	
	private $_format = '<div class="checkbox %s">%s <label for="%s">%s</label></div>';
	...
	public function generate() 
	{
		...
		{		
			for ($i=0; $i<sizeof($_options); $i++)
			{
				...
				$_checkbox = '<input type="checkbox" name="'.$this->handle.'[]" id="'.$_id.'" value="'.$_options[$i]['value'].'" '.$_checked.' '.$_attributes.'>';
				$_element .= vsprintf($this->_format, array($this->appearance, $_checkbox, $_id, $_options[$i]['name']));
			}
		}		
		if (intval($this->option_other) != 0)
		{
			...
			$_checkbox = '<input type="checkbox" name="'.$this->handle.'[]" id="'.$_id.'" value="option_other" '.$_checked.' '.$_attributes.'>';			
			$_element .= vsprintf($this->_format, array($this->appearance, $_checkbox, $_id, $this->option_other_value));
