...
*/
    ...
    public function Connect($argHostname = '', $argUsername = '', $argPassword = '', $argDatabaseName = '', $forceNew = false)
    {
        ...
        if ($this->debug) self::outp($this->host . ': ' . $err);
    ...
    */
    ...
    public function StartTrans($errfn = 'ADODB_TransMonitor')
    {
        ...
        if ($this->debug && $this->transCnt > 0) self::outp('Bad Transaction: StartTrans called within BeginTrans');
        $ok = $this->BeginTrans();
        $this->transOff = 1;
    ...
    public function CompleteTrans($autoComplete = true)
    {
        ...
        if ($this->_transOK && $autoComplete) {
            if (!$this->CommitTrans()) {
                $this->_transOK = false;
                if ($this->debug) self::outp('Smart Commit failed');
            } elseif ($this->debug) self::outp('Smart Commit occurred');
        } else {
            $this->_transOK = false;
            $this->RollbackTrans();
            if ($this->debug) self::outp('Smart Rollback occurred');
        }
    ...
    */
    public function FailTrans()
    {
        if ($this->debug)
            if ($this->transOff == 0) {
                self::outp('FailTrans outside StartTrans/CompleteTrans');
            } else {
                self::outp('FailTrans was called');
                adodb_backtrace();
            }
        $this->_transOK = false;
    }
    ...
    public function Affected_Rows()
    {
        ...
        if ($this->debug) self::outp('<p>Affected_Rows error</p>', false);
    ...
    */
    ...
    global $ADODB_CACHE;
        ...
        // no cached rs found
            if ($this->debug) {
                if (get_magic_quotes_runtime() && !$this->memCache) {
                    self::outp('Please disable magic_quotes_runtime - it corrupts cache files :(');
                }
                if ($this->debug !== -1) self::outp(" $md5file cache failure: $err (this is a notice and not an error)");
            }
        ...
        // ok, set cached object found
            ...
            if ($this->debug){
                ...
                self::outp(" $md5file reloaded, ttl=$ttl [ $s ]");
            }
        }
    ...
    {
        if ($this->debug) self::outp('BeginTrans: Transactions not supported for this driver');
...
*/
    ...
    class ADORecordSet_empty implements IteratorAggregate
    {
        ...
        public function PO_RecordCount() {return 0;}
        public function Close() {return true;}
        public function FetchRow() {return false;}
        public function FieldCount() { return 0;}
        public function Init() {}
        public function getIterator() {return new ADODB_Iterator_empty($this);}
        public function GetAssoc() {return array();}
    }
