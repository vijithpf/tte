...
class Zend_Locale
{
     ...
     * @param bool               $strict     (Optional) If true, no rerouting will be done when checking
     * @param bool               $compatible (DEPRECATED) Only for internal usage, brakes compatibility mode
     *
     * @return bool If the locale is known dependend on the settings
     */
    ...
    public static function findLocale($locale = null)
    {
        ...
        if (!self::isLocale($locale, true, false)) {
            if (!self::isLocale($locale, false, false)) {
                $locale = self::getLocaleToTerritory($locale);
     ...
     * @param bool               $strict (Optional) Strict preparation
     *
