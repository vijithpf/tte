...
class Concrete5_Helper_Date
{
    ...
    public function formatDate($value = 'now', $longDate = false, $timezone = 'user')
    {
        ...
        if ($longDate) {
            $format = defined('CUSTOM_DATE_APP_GENERIC_MDY_FULL') ?
                ...
                :
                t(/*i18n: Long date format: see http://www.php.net/manual/en/function.date.php */
                    'F j, Y');
        } else {
    ...
    public function formatTime($value = 'now', $withSeconds = false, $timezone = 'user')
    {
        ...
        if ($withSeconds) {
            $format = defined('CUSTOM_DATE_APP_GENERIC_TS') ?
                ...
                :
                t(/*i18n: Time format with seconds: see http://www.php.net/manual/en/function.date.php */
                    'g:i:s A');
        } else {
            $format =
                defined('CUSTOM_DATE_APP_GENERIC_T') ?
                    ...
                    :
                    t(/*i18n: Time format without seconds: see http://www.php.net/manual/en/function.date.php */
                        'g:i A');
        }
        ...
        if ($longDate) {
            if ($withSeconds) {
                $format = defined('CUSTOM_DATE_APP_GENERIC_MDYT_FULL_SECONDS') ?
                    ...
                    :
                    t(/*i18n: Long date format and time with seconds: see http://www.php.net/manual/en/function.date.php */
                        'F d, Y \\a\\t g:i:s A');
            } else {
                $format = defined('CUSTOM_DATE_APP_GENERIC_MDYT_FULL') ?
                    CUSTOM_DATE_APP_GENERIC_MDYT_FULL
                    :
                    t(/*i18n: Long date format and time without seconds: see http://www.php.net/manual/en/function.date.php */
                        'F d, Y \\a\\t g:i A');
            }
        } else {
            if ($withSeconds) {
                $format = t(/*i18n: Short date format and time with seconds: see http://www.php.net/manual/en/function.date.php */
                    'n/j/Y \\a\\t g:i:s A');
            } else {
                $format = defined('CUSTOM_DATE_APP_GENERIC_MDYT') ?
                    CUSTOM_DATE_APP_GENERIC_MDYT
                    :
                    t(/*i18n: Short date format and time without seconds: see http://www.php.net/manual/en/function.date.php */
                        'n/j/Y \\a\\t g:i A');
            }
        }
        ...
        switch ($days) {
            case 0:
                return t(/*i18n: %s is a time */
                    'Today at %s', $this->formatTime($zendDate, $withSeconds, $timezone));
            case 1:
                return t(/*i18n: %s is a time */
                    'Tomorrow at %s', $this->formatTime($zendDate, $withSeconds, $timezone));
            case -1:
                return t(/*i18n: %s is a time */
                    'Yesterday at %s', $this->formatTime($zendDate, $withSeconds, $timezone));
            default:
        ...
        switch ($formatName) {
            case 'FILENAME':
                return defined("CUSTOM_DATE_APP_$formatName") ?
                    ...
                    :
                    t(/*i18n: Used when dates are used to start filenames: see http://www.php.net/manual/en/function.date.php */
                        'd-m-Y_H:i_');
    ...
    public function getJQueryUIDatePickerFormat()
    {
        ...
        if (defined('CUSTOM_DATE_APP_GENERIC_MDY')) {
            ...
            $phpFormat = t(/*i18n: Short date format: see http://www.php.net/manual/en/function.date.php */
                'n/j/Y');
        }
     ...
     * @return string
     */
    public function getFormattedDate($date, $mask = null, $type = 'system')
    {
        if (ENABLE_USER_TIMEZONES && $type == 'user') {
            $date = $this->getLocalDateTime($date);
        }
        ...
        return $this->date($mask, strtotime($date));
    }
}
