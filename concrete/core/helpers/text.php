...
class Concrete5_Helper_Text
{
    ...
    public function replaceWhitespace($str, $replaceWith = ' ')
    {
        return preg_replace('!\s+!', $replaceWith, $str);
    }

    public function beautify($input, $autoLink = false)
    {
        $input = e($input);
        if ($autoLink) {
            $input = $this->autolink($input, 1);
        }
