...
class Concrete5_Controller_Block_Guestbook extends BlockController
{
     ...
     * @return bool
     */
    public function getDisplayGuestBookForm()
    {
     ...
     */
    public function action_form_save_entry()
    {
        $ip = Loader::helper('validation/ip');
        if (!$ip->check()) {
            $this->set('invalidIP', $ip->getErrorMessage());
     ...
     * @return array
     */
    public function getEntries($order = 'ASC')
    {
        $bo = $this->getBlockObject();
        $c  = Page::getCurrentPage();
     ...
     * @return bool
     */
    public function action_loadEntry()
    {
     ...
     */
    public function action_removeEntry()
    {
        $ip = Loader::helper('validation/ip');
        if (!$ip->check()) {
            $this->set('invalidIP', $ip->getErrorMessage());
     ...
     * deltes a given Entry, sets the response message for use in the view.
     */
    public function action_approveEntry()
    {
        $ip = Loader::helper('validation/ip');
        if (!$ip->check()) {
            $this->set('invalidIP', $ip->getErrorMessage());
     ...
     * deltes a given Entry, sets the response message for use in the view.
     */
    public function action_unApproveEntry()
    {
        $ip = Loader::helper('validation/ip');
        if (!$ip->check()) {
            $this->set('invalidIP', $ip->getErrorMessage());
