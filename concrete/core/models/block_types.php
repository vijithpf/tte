...
class Concrete5_Model_BlockTypeList extends Object
{
    ...
    public static function resetBlockTypeDisplayOrder($column = 'btID')
    {
        ...
        foreach ($blockTypes as $bt) {
            if ($bt['btIsInternal']) {
                $db->Execute($stmt, [0, $bt['btID']]);
            } else {
                $db->Execute($stmt, [$btDisplayOrder, $bt['btID']]);
                $btDisplayOrder++;
            }
        }
    }
...
class Concrete5_Model_BlockType extends Object
{
    ...
    public static function getByHandle($handle)
    {
        ...
        $bt = BlockType::get('btHandle = ?', [$handle]);
        if (is_object($bt)) {
    ...
    public function getCount($ignoreUnapprovedVersions = false)
    {
        ...
        if ($ignoreUnapprovedVersions) {
            ...
            $count = $db->GetOne("SELECT count(btID) FROM Blocks WHERE btID = ?", [$this->btID]);
        }
    ...
    public function installBlockType($btHandle, $btID = 0)
    {
        ...
        if ($btID == 0) {
            ...
            $db  = Loader::db();
            $cnt = $db->GetOne("SELECT btID FROM BlockTypes WHERE btHandle = ?", [$btHandle]);
            if ($cnt > 0) {
                return false;
            }
        }
    ...
    protected function doInstallBlockType($btHandle, $bt, $dir, $btID = 0, $dirDbXml)
    {
        ...
        if (file_exists($dir . '/' . $btHandle . '/' . FILENAME_BLOCK_CONTROLLER)) {
            ...
            if ($btID > 0) {
                $btd->btID      = $btID;
                $btDisplayOrder = $db->GetOne('SELECT btDisplayOrder FROM BlockTypes WHERE btID = ?', [$btID]);
                if (!$btDisplayOrder) {
                    $btDisplayOrder = 0;
                }
    ...
    {
        $db = Loader::db();
        $r  = $db->Execute('SELECT cID, cvID, b.bID, arHandle FROM CollectionVersionBlocks cvb INNER JOIN Blocks b ON b.bID = cvb.bID WHERE btID = ?', [$this->getBlockTypeID()]);
        while ($row = $r->FetchRow()) {
        ...
        $ca->delete('blockTypeByHandle', $this->btHandle);
        $ca->delete('blockTypeList', false);
        $db->Execute("DELETE FROM BlockTypes WHERE btID = ?", [$this->btID]);
    ...
    public function update($data)
    {
        ...
        $db->Execute('UPDATE BlockTypes SET btHandle = ?, btName = ?, btDescription = ? WHERE btID = ?', [$btHandle, $btName, $btDescription, $this->btID]);
    ...
    public function add($data, $c = false, $a = false)
    {
        ...
        $v = [$_POST['bName'], $bDate, $bDate, $bIsActive, $btID, $uID];
