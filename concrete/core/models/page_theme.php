...
class Concrete5_Model_PageTheme extends Object
{
            ...
            foreach ($styles as $p) {
                if ($p->getType() == PageThemeEditableStyle::TSTYPE_CUSTOM) {
                    $contents = preg_replace("/\/\*[\s]?customize_" . $p->getHandle() . "[\s]?\*\/(.*)\/\*[\s]?customize_" . $p->getHandle() . "[\s]?\*\//i",
                                             '/* customize_' . $p->getHandle() . ' */ ' . $p->getValue() . ' /* customize_' . $p->getHandle() . ' */', $contents);
                } else {
    ...
    {
        return $this->ptURL . '/' . PageTheme::FILENAME_TYPOGRAPHY_CSS;
    }
    ...
    {
        if (self::$siteTheme === null) {
            $c               = Page::getByID(HOME_CID);
            self::$siteTheme = PageTheme::getByID($c->getCollectionThemeID());
        }
