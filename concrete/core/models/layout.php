...
class Concrete5_Model_Layout extends Object
{
    ...
    {
        return intval($this->layoutID);
    }

    public function setAreaNameNumber($v)
    {
        $this->areaNameNumber = intval($v);
    }

    public function getAreaNameNumber()
    {
        return intval($this->areaNameNumber);
    }

    public function getLayoutNameDivider()
    {
        return ' : ';
    }

    public function getLayoutNameTxt()
    {
        return 'Layout';
    }
    ...
    {
        return $this->getAreaHandle() . $this->getLayoutNameDivider() . $this->getLayoutNameTxt() . ' ' . $this->getAreaNameNumber();
    }

    public function getLayoutPresetId()
    {
        return intval($this->lpID);
    }
    ...
    public function fill($params = array('layoutID' => 0, 'type' => 'table', 'rows' => 3, 'columns' => 3, 'breakpoints' => array(), 'locked' => 0, 'lpID' => 0, 'lpName' => '', 'spacing' => 0))
    {
        ...
        if (intval($params['layout_rows'])) {
            $this->rows = intval($params['layout_rows']);
        } else {
            $this->rows = (intval($params['rows']) < 1) ? 1 : $params['rows'];
        }

        if (intval($params['layout_columns'])) {
            $this->columns = intval($params['layout_columns']);
        } else {
            $this->columns = (intval($params['columns']) < 1) ? 3 : $params['columns'];
        }

        if (intval($params['areaNameNumber'])) {
            $this->areaNameNumber = intval($params['areaNameNumber']);
        }

        $this->lpID   = intval($params['lpID']);
        $this->lpName = $params['lpName'];

        if (strlen($params['spacing'])) {
            $this->spacing = $params['spacing'];
        }

        if (!is_array($params['breakpoints']) && strlen(trim($params['breakpoints']))) {
            $this->breakpoints = explode(',', $params['breakpoints']);
        } elseif (is_array($params['breakpoints']) && (count($params['breakpoints']) || $this->columns == 1)) {
    ...
    {
        if (!$this->areaObj) {
            throw new Exception(t('Error: no area assigned to layout'));
        }
    ...
    public function save()
    {
        if (!is_array($this->breakpoints)) {
            $this->breakpoints = explode(',', $this->breakpoints);
        }
        $vals = array(intval($this->columns), intval($this->rows), intval($this->locked), implode(',', $this->breakpoints), $this->spacing);
    ...
    public function display($c = null, $a = null)
    {
        ...
        if (!in_array($this->type, $this->layoutTypes)) {
    ...
    */
    ...
    protected function resetCellNumber()
    {
        $this->cellNum = 0;
    }
