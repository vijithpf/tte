...
class UspSectionBlockController extends BlockController {
	
	protected $btName = 'USP Section';
	protected $btDescription = '';
	protected $btTable = 'btDCUspSection';
	...
	public function getSearchableContent() {
		...
		$content[] = $this->field_3_wysiwyg_content;
	...
	public function edit() {
		...
		$this->set('field_3_wysiwyg_content', $this->translateFromEditMode($this->field_3_wysiwyg_content));
	...
	public function save($args) {
		...
		$args['field_3_wysiwyg_content'] = $this->translateTo($args['field_3_wysiwyg_content']);
...
//WYSIWYG HELPER FUNCTIONS (COPIED FROM "CONTENT" BLOCK):
	function translateFromEditMode($text) {
		...
		$text = preg_replace_callback(
			'/{CCM:FID_([0-9]+)}/i',
			array('UspSectionBlockController', 'replaceFileIDInEditMode'),
			$text);


		$text = preg_replace_callback(
			'/{CCM:FID_DL_([0-9]+)}/i',
			array('UspSectionBlockController', 'replaceDownloadFileIDInEditMode'),
			$text);
	...
	function translateFrom($text) {
		...
		$text = preg_replace_callback(
			'/{CCM:CID_([0-9]+)}/i',
			array('UspSectionBlockController', 'replaceCollectionID'),
			$text);

		$text = preg_replace_callback(
			'/<img [^>]*src\s*=\s*"{CCM:FID_([0-9]+)}"[^>]*>/i',
			array('UspSectionBlockController', 'replaceImageID'),
			$text);

		// now we add in support for the files that we view inline
		$text = preg_replace_callback(
			'/{CCM:FID_([0-9]+)}/i',
			array('UspSectionBlockController', 'replaceFileID'),
			$text);
		...
		$text = preg_replace_callback(
			'/{CCM:FID_DL_([0-9]+)}/i',
			array('UspSectionBlockController', 'replaceDownloadFileID'),
			$text);
