...
class ImageContentBlockBlockController extends BlockController {
	
	protected $btName = 'Image Content Block';
	protected $btDescription = '';
	protected $btTable = 'btDCImageContentBlock';
	...
	public function add() {
		...
		$field_2_default_content = '<h2>The title</h2>
<p>This is the description.</p>';
		$this->set('field_2_wysiwyg_content', $field_2_default_content);
	}
...
//WYSIWYG HELPER FUNCTIONS (COPIED FROM "CONTENT" BLOCK):
	function translateFromEditMode($text) {
		...
		$text = preg_replace_callback(
			'/{CCM:FID_([0-9]+)}/i',
			array('ImageContentBlockBlockController', 'replaceFileIDInEditMode'),
			$text);


		$text = preg_replace_callback(
			'/{CCM:FID_DL_([0-9]+)}/i',
			array('ImageContentBlockBlockController', 'replaceDownloadFileIDInEditMode'),
			$text);
	...
	function translateFrom($text) {
		...
		$text = preg_replace_callback(
			'/{CCM:CID_([0-9]+)}/i',
			array('ImageContentBlockBlockController', 'replaceCollectionID'),
			$text);

		$text = preg_replace_callback(
			'/<img [^>]*src\s*=\s*"{CCM:FID_([0-9]+)}"[^>]*>/i',
			array('ImageContentBlockBlockController', 'replaceImageID'),
			$text);

		// now we add in support for the files that we view inline
		$text = preg_replace_callback(
			'/{CCM:FID_([0-9]+)}/i',
			array('ImageContentBlockBlockController', 'replaceFileID'),
			$text);
		...
		$text = preg_replace_callback(
			'/{CCM:FID_DL_([0-9]+)}/i',
			array('ImageContentBlockBlockController', 'replaceDownloadFileID'),
			$text);
