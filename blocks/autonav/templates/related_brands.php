...
<section class="all-brands">
  <div class="container">
    ...
    <?php
      foreach ($navItems as $ni) {
        $thumbnail = $ni->cObj->getAttribute('thumbnail_image');
        $url = $ni->url;
        $name = $ni->name;
        $thumbnailUrl = $thumbnail->getURL();
        //if its not current page then show
        if (!$ni->isCurrent) {
        ?>
          <div class="col-sm-4">
            ...
            <a href="<?php echo $url; ?>" class="btn btn-underline btn-clr-purple">See All Products</a>
          </div>
        <?php
          }
        } ?>
